...
async function sshSftp(opts) {
  ...
  try {
    ...
    if (opts.ignore && opts.ignore.length > 0) {
      ...
      for (const o of ls) {
        spinner.text = `Uploading file ${o.localPath} to ${o.remotePath}`
...
/**
 * @param {Options} opts
 * @param {{d:boolean,u:boolean}} lsOpts
 */
  ...
  try {
    ...
    if (lsOpts.d) {
      if (opts.cleanRemoteFiles) {
        ...
        console.log(`Delete ${opts.remotePath} files:`)
        for (const s of ls) {
          console.log(`  - ${s}`)
        }
      }
